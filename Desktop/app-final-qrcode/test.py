...
def update_chat_display(partial_message):
    ...
    for role, message in st.session_state.chat_history:
        if role == "assistant":
            chat_html += (
                ...
                f"<strong>EeeBee:</strong> {message}</div>"
            )
        else:
            chat_html += (
    ...
    if partial_message:
        chat_html += (
            "<div style='text-align: left; background-color: #e0e7ff; color: #000; padding: 8px; border-radius: 8px; margin-bottom: 5px;'>"
            f"<strong>EeeBee:</strong> {partial_message}</div>"
        )
    chat_html += "</div>"
    chat_html += """
    <script>
    ...
    try:
        with st.spinner("EeeBee is thinking..."):
            ...
            for chunk in response:
                ...
                update_chat_display(partial_message)
            st.session_state.chat_history.append(("assistant", partial_message))
    except Exception as e:
...
def handle_teacher_commands(user_input: str):
        ...
        if student_info and student_info.get("Students"):
            ...
            for concept in concepts:
                ...
                concept_stats.append(
                    f"- {concept['ConceptText']}: {concept['ClearedStudentCount']}/{concept['AttendedStudentCount']} students cleared ({cleared_percent:.1f}%)"
                )
            concept_overview = "\n".join(concept_stats)
            return (
        ...
        if hasattr(st.session_state, 'current_batch_students'):
            ...
            for student in students:
                if student['ClearedConceptCount'] == student['TotalConceptCount']:
                    ...
                    partial_progress.append(f"- {student['FullName']} ({student['ClearedConceptCount']}/{student['TotalConceptCount']} concepts cleared)")
                else:
    ...
    if hasattr(st.session_state, 'current_batch_students'):
        ...
        if selected_student:
            ...
            if student_concepts:
                weak_concepts_details = [format_concept_details(concept) for concept in student_concepts.get('WeakConcepts_List', [])]
                cleared_concepts_details = [format_concept_details(concept) for concept in student_concepts.get('ClearedConcepts_List', [])]
            total_questions = 0
            total_correct = 0
            total_time = 0
