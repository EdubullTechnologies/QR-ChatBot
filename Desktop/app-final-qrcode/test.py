...
import openai  # If using Whisper for speech-to-text
...
def display_chat(user_name: str):
    ...
    # Add voice mode toggle at the bottom
    col1, col2 = st.columns([4, 1])
    with col2:
        voice_enabled = st.toggle("🎤 Voice Mode", key="voice_mode")
    
    if voice_enabled:
        if st.button("Hold to Speak", key="record_button"):
            with st.spinner("Recording... (5 seconds)"):
                audio, fs = record_audio()
                st.session_state.audio_data = (audio, fs)
                
        if st.session_state.audio_data:
            audio, fs = st.session_state.audio_data
            user_input = speech_to_text(audio, fs)
            if user_input:
                handle_user_input(user_input)
                st.session_state.audio_data = None
    ...
    for role, message in st.session_state.chat_history:
        if role == "assistant" and voice_enabled:
...
def text_to_speech(text, voice='nova'):
    ...
    # Requires ELEVEN_LABS_API_KEY in secrets
    url = f"https://api.elevenlabs.io/v1/text-to-speech/{voice}"
    
    headers = {
        "Accept": "audio/mpeg",
        "Content-Type": "application/json",
        "xi-api-key": st.secrets["ELEVEN_LABS_API_KEY"]
    }
