...
# For demonstration, we do not set the key directly here.
# Provide your own, e.g.: openai.api_key = "sk-xxx"
...
def baseline_testing_report():
    ...
    if s_skills:
        ...
        # Horizontal bar chart with x => RightAnswerPercent, y => SubjectSkillName
        ...
        if chosen_concept_text:
            ...
            if st.button("Generate Exam Questions"):
                ...
                with st.spinner("Generating exam questions... Please wait."):
                    try:
                        # Adjust model as needed (e.g., gpt-4o)
...
def login_screen():
    """
    Shows a login screen if the user is not yet authenticated.
    Checks URL params for E= (English) or T= (Math/Science).
    On successful login, stores user info in session_state, so that
    the user remains logged in unless they click 'Logout'.
    """
    # If user is already authenticated, skip the login screen
    ...
    if st.button("🚀 Login and Start Chatting!") and not st.session_state.is_authenticated:
        if topic_id is None or api_url is None:
            st.warning("Please ensure correct E or T parameter is provided in the URL.")
            return
        ...
        if not st.session_state.is_english_mode:
            # For Non-English (Math/Science), we need user_type
        ...
        try:
            with st.spinner("🔄 Authenticating..."):
                ...
                if auth_data.get("statusCode") == 1:
                    ...
                    # Attempt to capture SubjectID if returned
                    ...
                    # On success, re-run to go to main screen
                    st.rerun()
                else:
...
def add_initial_greeting():
    """
    Adds an initial assistant greeting message if the chat is empty.
    """
...
def get_gpt_response(user_input):
    ...
    try:
        with st.spinner("EeeBee is thinking..."):
            ...
            # Adjust your model name as needed (gpt-4o, etc.)
            gpt_response = openai.ChatCompletion.create(
                model="gpt-4o",
...
def main_screen():
    """
    Displays the main app screen once the user is authenticated.
    Shows different tabs depending on whether user is a Teacher or Student,
    and whether it is English mode or Non-English mode.
    """
    # Safe to assume we have user info
    user_name = st.session_state.auth_data['UserInfo'][0]['FullName']
    topic_name = st.session_state.auth_data.get('TopicName', 'Unknown Topic')
...
# ----------------------------------------------------------------------------
def main():
    # If user is already authenticated, go directly to main screen
    if st.session_state.is_authenticated and st.session_state.auth_data is not None:
        main_screen()
        st.stop()
    else:
        # Otherwise, show the login screen
        login_screen()
        # If user became authenticated in login_screen, display main screen
        if st.session_state.is_authenticated and st.session_state.auth_data is not None:
            st.rerun()
