...
if "voice_mode" not in st.session_state:
    st.session_state.voice_mode = False  # Are we currently recording?
...
# ------------------------------
# Helper function: Convert LaTeX to image for PDFs
# ------------------------------
...
def generate_learning_path(concept_text):
    prompt = (
        ...
        f"1. **Introduction to the Concept**: Importance and applications.\n"
        f"2. **Step-by-Step Learning**.\n"
        f"3. **Engagement**: interactive activities.\n"
        f"4. **Real-World Applications**.\n"
        f"5. **Practice Problems**.\n"
        f"Use LaTeX for any math expressions ($...$ or $$...$$)."
    )
    ...
    if st.session_state.teacher_weak_concepts:
        ...
        if chosen_concept_text:
            ...
            if st.button("Generate Exam Questions"):
                ...
                prompt = (
                    f"You are an educational AI assistant helping a teacher. The teacher wants to create exam questions "
                    f"for the concept '{chosen_concept_text}'.\n"
...
- Ensure that all mathematical expressions are enclosed within LaTeX delimiters ($...$ or $$...$$)."""
    else:
...
def load_concept_content():
    ...
    selected_concept_name = next(
        (concept['ConceptText'] for concept in st.session_state.auth_data['ConceptList']
         if concept['ConceptID'] == selected_concept_id),
        "Unknown Concept"
    )
    content_payload = {
        'TopicID': st.session_state.topic_id,
    ...
    try:
        with st.spinner("🔄 Fetching concept content..."):
            ...
            prompt = (
                f"Provide a concise and educational description of the concept '{selected_concept_name}' "
                f"to help students understand it better. "
                f"Ensure that all mathematical expressions are enclosed within LaTeX delimiters."
            )
            gpt_response = openai.ChatCompletion.create(
                model="gpt-4o-mini",
...
def audio_callback(frame):
    """Receive audio frames and store them in session state."""
    st.session_state.audio_frames.append(frame.to_bytes())

def transcribe_audio(audio_frames):
    """Use speech_recognition to convert raw audio frames into text."""
    recognizer = sr.Recognizer()
    audio_data = b"".join(audio_frames)
    sample_rate = 48000  # typical for streamlit-webrtc
...
def microphone_ui():
    """
    Displays the recorder if voice_mode is True:
      - webrtc_streamer in SENDONLY mode
    ...
    if st.button("Stop Recording"):
        ...
        if st.session_state.audio_frames:
            with st.spinner("Transcribing..."):
                result = transcribe_audio(st.session_state.audio_frames)
            st.session_state.audio_frames.clear()
            st.success(f"Voice input: {result}")
            if result.strip():
                # Add the recognized text as a user message
                st.session_state.chat_history.append(("user", result))
                # Generate the AI response
                get_gpt_response(result)
...
def main_screen():
    ...
    if st.session_state.is_teacher:
        ...
        with tabs[0]:
            ...
            # Chat input row: Text input + Mic Button
            col_input, col_mic = st.columns([0.85, 0.15])
            with col_input:
                user_text = st.text_input("Type your question here...", key="chat_input")
            with col_mic:
                if st.button("🎤", use_container_width=True):
                    st.session_state.voice_mode = True

            # If we're in voice_mode, show the mic UI
        ...
        if st.session_state.is_english_mode:
            ...
            with tab_chat:
                ...
                # Chat input row for student
                col_input, col_mic = st.columns([0.85, 0.15])
                with col_input:
                    user_text = st.text_input("Enter your question about the topic", key="student_chat_input")
                with col_mic:
                    if st.button("🎤", use_container_width=True):
                        st.session_state.voice_mode = True
                ...
                if st.button("Send (Student)"):
                    if user_text.strip():
                        handle_user_input(user_text)
                    st.session_state.student_chat_input = ""

        else:
            tab_chat, tab_lp, tab_concepts = st.tabs(["💬 Chat", "🧠 Learning Path", "📚 Concepts"])
            with tab_chat:
                st.subheader("Chat with your EeeBee AI buddy", anchor=None)
                add_initial_greeting()
                ...
                col_input, col_mic = st.columns([0.85, 0.15])
                with col_input:
                    user_text = st.text_input("Enter your question about the topic", key="non_english_chat_input")
                with col_mic:
                    if st.button("🎤", use_container_width=True):
                        st.session_state.voice_mode = True
                ...
                if st.button("Send (Non-English)"):
                    if user_text.strip():
                        handle_user_input(user_text)
                    st.session_state.non_english_chat_input = ""
